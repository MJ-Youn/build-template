...
configure_compose() {
    ...
    # Strategy 2,3 등에서 동적 이미지 태그가 없을 경우 latest 기본 적용
    DOCKER_IMAGE="${APP_NAME}:latest"
    if [ -f "$IMAGE_TAR" ]; then
         # Tar 기반 로드 시 이미지명 확인 (간단하게 앱이름 매핑)
         DOCKER_IMAGE="${APP_NAME}:latest"
    else
         # 레지스트리에서 가져오거나 로컬 빌드 이미지일 경우 
         # docker-compose.yml 안의 image: ${DOCKER_IMAGE:-app:latest} 기본값 작동
         DOCKER_IMAGE=""
    fi
...
# Generated by install_docker_service.sh
# ==========================================================
APP_NAME=$APP_NAME
LOG_PATH=$LOG_PATH
DEST_DIR=$DEST_DIR
EOF
    ...
    # [추가] 빈 폴더 마운트로 인한 컨테이너 내부 config 초기화 방지
    # 호스트의 config 폴더가 없거나 비어있으면 초기 파일들(application.yml 등)을 이미지에서 뽑아와 채워 넣습니다.
    local CONFIG_DIR="$DEST_DIR/config"
    mkdir -p "$CONFIG_DIR"
    
    if [ -z "$(ls -A "$CONFIG_DIR")" ]; then
        log_step "초기 Host Config 파일(application.yml 등) 생성 중..."
        # 임시 컨테이너를 띄워 내부의 /app/config 내용물을 호스트 볼륨으로 복사 (오류 무시)
        local RUN_IMAGE="${DOCKER_IMAGE:-${APP_NAME}:latest}"
        docker run --rm -v "$CONFIG_DIR:/tmp_config" "$RUN_IMAGE" sh -c "cp -r /app/config/* /tmp_config/ 2>/dev/null || true"

...
if [ -f "$APP_CONFIG" ]; then
    ...
    if [ -d "config.profiles" ]; then
        find config.profiles -name "application-*.yml" -exec "${SED_CMD[@]}" "s/name: $CURRENT_PROJECT_NAME/name: $NEW_PROJECT_NAME/g" {} + 2>/dev/null || true
...
echo "변경된 설정을 확인해주세요."
echo "-----------------------------------------"
echo "이 스크립트($0) 및 빌드/배포에 불필요한 초기화 파일들이 삭제됩니다."

# 템플릿 배포 스크립트 삭제 (프로젝트가 초기화된 후에는 다른 프로젝트로 템플릿을 복사할 일이 없으므로)
TEMPLATE_SCRIPT="apply_build_deploy_template.sh"
if [ -f "$TEMPLATE_SCRIPT" ]; then
    rm -f "$TEMPLATE_SCRIPT"
    echo "🗑️  불필요한 스크립트 삭제 완료: $TEMPLATE_SCRIPT"
fi
